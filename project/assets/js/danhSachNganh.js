...
function ganDanhSachNganhV2(target) {
    let lop = target.closest('td').nextElementSibling.children[0];
    let danhSachNganh = layTenNganh(target.value);
    let output = "";
    for (let i = 0; i < danhSachNganh.length; i++) {
        if (i == 0)
            output += "<option value='" + danhSachNganh[i] + "'>" + danhSachNganh[i] + "</option>";
    ...
    lop.innerHTML = output;
}

function layMaTruong() {
    return JSON.stringify(maTruong);
}
